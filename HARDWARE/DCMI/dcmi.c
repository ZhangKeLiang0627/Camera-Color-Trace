...
TARGET_CONDI Condition = {0, 240, 0, 15, 0, 20, 5, 5, 50, 50}; // 黑色
RESULT Result;
uint8_t temp_result;
...
uint16_t Color_Block_y1, Color_Block_y2, Color_Block_x1, Color_Block_x2; // 色块的四边框坐标值
...
void Find_color_block_pos_y1(void)
{
	...
	for (uint16_t i = 0; i < 80 * 60; i++) // 上半框横轴查找 // 以2行为步进
	{
		if (i % 80 == 0 && i != 0)
		{
			temp += 161;
...
void Find_color_block_pos_x1(void)
{
	...
	for (uint16_t i = 0; i < 80 * 80; i++) // 上半框横轴查找
	{
		if (i % 80 == 0 && i != 0)
		{
			temp -= 79 * 240;
...
void Template_cut(void) // 模板裁剪
{
	...
	uint16_t ErrX = Color_Block_x2 - Color_Block_x1; // x上下轴间距
	uint16_t ErrY = Color_Block_y2 - Color_Block_y1; // y上下轴间距
	float SrcX = 0;
	float SrcY = 0;

	if (ErrX < 80 && ErrY < 120)
	{
		for (uint16_t i = 0; i < ErrX * ErrY; i++) // 对240*120的中心区域80*100进行裁剪
		{
			if (i % ErrX == 0 && i != 0)
			{
				temp += 1 + 240 - ErrX;
